...
def springEuler(x_0, v_0, h, numSteps):
    ...
    plt.savefig('explicit_spring_motion.jpg')
...
def plotError(x_0, v_0, h, numSteps): 
    ...
    plt.savefig('explicit_spring_error.jpg')
...
def plotEnergyEvolution(x_0, v_0, h, numSteps):
    ...
    plt.savefig('energy_evolution_explicit.jpg')
...
def implicitEuler(x_0, v_0, h, numSteps):
    ...
    plt.savefig('energy_evolution_implicit_explicit.jpg')
    figError = plt.figure()
    plt.plot(tArr, np.absolute(xArrImp - xArrAnal), 'k-', \
        tArr, np.absolute(xArrExp - xArrAnal), 'b-')
    plt.savefig('implicit_global_error.jpg')
...
def phaseSpace(x_0, v_0, h, numSteps):
    ...
    plt.savefig('implicit_phasespace.jpg')
    fig2 = plt.figure()
    plt.plot(xArrExp, vArrExp, 'b-')
    plt.savefig('explicit_phasespace.jpg')
    fig3 = plt.figure()
    plt.plot(xArrSymp, vArrSymp, 'r-')
    plt.savefig('symplectic_phasespace.jpg')
...
def plotSymplecticEnergyEvolution(x_0, v_0, h, numSteps):
    ...
    plt.plot(tArr, xArrSymp**2 + vArrSymp**2, 'k-', \
        tArr, xArrAnal**2 + vArrAnal**2, 'b-')
    plt.savefig('symplectic_energy_evolution.jpg')

usage = 'usage: python %s Lab Problem Number'
if len(sys.argv) != 2:
    print >> sys.stderr, usage % sys.argv[0]
    sys.exit(1) 
if sys.argv[1] == '1':
    springEuler(5, 2, .0001, 100000)
if sys.argv[1] == '2':
    plotError(5, 2, .0001, 100000)
if sys.argv[1] == '3':
    plotTruncationError(5, 2, .0001, 10)
if sys.argv[1] == '4':
    plotEnergyEvolution(5, 2, .0001, 100000)
if sys.argv[1] == '5':
    implicitEuler(5, 2, .0001, 100000)
if sys.argv[1] == 'phasespace':
    phaseSpace(5, 2, .0001, 100000)
if sys.argv[1] == '2.3':
    plotSymplecticEnergyEvolution(5, 2, .0001, 100000)
